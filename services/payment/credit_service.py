...
from api.utils.credit_models import (
    UserCreditDB, CreditTransactionDB, CreditPackageDB, 
    ReferralDB, CreditUsageRuleDB, SubscriptionCreditDB
)
...
class CreditService:
    ...
    def get_user_credits(self, user_id: int) -> UserCreditDB:
        ...
        db = SessionLocal()
        try:
            user_credits = db.query(UserCreditDB).filter(
                UserCreditDB.user_id == user_id
            ).first()
    ...
    def consume_credits(
        ...
        try:
            ...
            if user_credits.balance < amount:
                return False, f"Insufficient credits. Need {amount}, have {user_credits.balance}"
            ...
            logger.info(f"[CreditService] User {user_id} spent {amount} credits for {action}")
    ...
    def get_transaction_history(
        ...
        try:
            ...
            ).order_by(CreditTransactionDB.created_at.desc()).offset(offset).limit(limit).all()
    ...
    def get_credit_packages(self) -> list:
        ...
        db = SessionLocal()
        try:
            packages = db.query(CreditPackageDB).filter(
                CreditPackageDB.is_active == True
            ).order_by(CreditPackageDB.price_cents.asc()).all()
    ...
    def apply_referral_code(self, referrer_id: int, referral_code: str) -> tuple[bool, str]:
        ...
        try:
            ...
            logger.info(f"[CreditService] Referral applied: {referrer_id} used code {referral_code}")
    ...
    def check_and_consume(
        ...
        if not self.has_sufficient_credits(user_id, cost):
            return False, f"Insufficient credits. Need {cost}, have {self.get_balance(user_id)}"
