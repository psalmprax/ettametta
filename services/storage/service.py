...
class StorageService:
    def __init__(self):
        ...
        if not self.boto3_available:
            self.logger.warning(
                "StorageService: boto3 not available. Cloud storage features disabled."
            )
    ...
    def _get_client(self):
        ...
        secret_key = get_secret("storage_secret_key") or get_secret(
        ...
        try:
            ...
            client_kwargs = {"service_name": "s3", "region_name": self.region}
    ...
    def upload_to_cloud(
        ...
        if not s3_client:
            logging.error(
                "[StorageService] Cannot upload to cloud: s3_client not initialized."
            )
            return None
