...
def generate_cogvideo_api(prompt: str, output_dir: str, height: int = 480, width: int = 720, model_size: str = "2b") -> Tuple[str, str]:
...
def generate_cogvideo_dummy(output_dir: str) -> Tuple[str, str]:
    """Fallback dummy for CogVideoX"""
    job_id = f"cogx_dummy_{int(time.time())}"
    output_path = os.path.join(output_dir, f"{job_id}.mp4")
