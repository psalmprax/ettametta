...
def generate_hunyuan_api(prompt: str, output_dir: str, height: int = 480, width: int = 832) -> Tuple[str, str]:
    """Generate video using Hunyuan API on Remote GPU"""
    job_id = f"hun_api_{int(time.time())}"
    output_path = os.path.join(output_dir, f"{job_id}.mp4")
    os.makedirs(output_dir, exist_ok=True)
    
    print(f"📡 Attempting Hunyuan remote generation via {settings.RENDER_NODE_URL}...", flush=True)
...
def generate_hunyuan_dummy(output_dir: str) -> Tuple[str, str]:
    """Fallback dummy for Hunyuan"""
    job_id = f"hun_dummy_{int(time.time())}"
    output_path = os.path.join(output_dir, f"{job_id}.mp4")
