...
function IntegrityAuditPanel({ isAuditing, auditResult, triggerAudit }: {
...
function VulnerabilityScanPanel({ isScanning, scanResult, triggerVulnerabilityScan }: {
...
function SecurityEventsLog({ securityEvents }: { readonly securityEvents: any[] }) {
  return (
    <div className="p-6 rounded-[32px] bg-[#0F0F11]/60 border border-white/5 space-y-6">
      <div className="flex items-center justify-between">
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <StatusCard icon={Activity} label="Threat Level" value={securityStatus?.threat_level || "NOMINAL"} color={threatColor(securityStatus?.threat_level)} />
        <StatusCard icon={ShieldCheck} label="System Integrity" value={securityStatus?.system_integrity || "NOMINAL"} color={threatColor(securityStatus?.system_integrity)} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <IntegrityAuditPanel isAuditing={isAuditing} auditResult={auditResult} triggerAudit={triggerAudit} />
        <VulnerabilityScanPanel isScanning={isScanning} scanResult={scanResult} triggerVulnerabilityScan={triggerVulnerabilityScan} />
      </div>

      <SecurityEventsLog securityEvents={securityEvents} />
    </div>
  );
}
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-3 mt-4">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-white/3 border border-white/5">
                ...
                <span className={cn("text-sm font-bold", threatColor(securityStatus?.threat_level))}>
                  {securityStatus?.threat_level || "NOMINAL"}
                </span>
              </div>
              <div className="flex items-center justify-between p-3 rounded-xl bg-white/3 border border-white/5">
                <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">System Integrity</span>
                <span className={cn("text-sm font-bold", threatColor(securityStatus?.system_integrity))}>
