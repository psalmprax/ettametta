...
def _generate_video(job_id: str, request: GenerationRequest):
    ...
    if pipe is None:
        raise RuntimeError(
            "Diffusers pipeline not loaded. Install diffusers and configure MODEL_ID."
        )
...
def _animate_persona(job_id: str, request: PersonaRequest):
    """Background task to generate persona animation via SadTalker/LivePortrait"""
    output_path = os.path.join(OUTPUT_DIR, f"{job_id}.mp4")
    logger.info(
        f"Animating persona for {request.image_url} speaking '{request.text[:30]}'"
    )

    # Attempt SadTalker integration
    try:
        import requests
        import tempfile

        # Download source image
        img_resp = requests.get(request.image_url, timeout=30)
        ...
        # Try SadTalker if available
        try:
            from sadtalker import SadTalkerPipeline

            pipeline = SadTalkerPipeline()
            pipeline.generate(
                source_image=tmp_img_path,
                driven_audio=None,
                driven_text=request.text,
                output_path=output_path,
            )
            logger.info(f"SadTalker persona animation complete: {output_path}")
        except ImportError:
            # SadTalker not installed - raise clear error
            raise RuntimeError(
                "SadTalker not installed. Install via: pip install sadtalker. "
                "Persona animation requires a face animation model."
            )
        finally:
            import os as _os
