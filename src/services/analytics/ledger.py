...
class PerformanceLedger:
    ...
    def log_entry(self, video_id: str, predicted: float, actual: float, model_mae: float):
        ...
        try:
            ...
            logger.info(f"📗 [Ledger] Logged {video_id}: Err={entry['error']:.4f}")
    ...
    def get_accuracy_report(self) -> dict:
        ...
        if not os.path.exists(self.ledger_path):
            return {"accuracy": "no_data"}
            
        with open(self.ledger_path, 'r') as f:
            entries = json.load(f)
        
        if not entries: return {"accuracy": "no_data"}
