...
class RunwaySkill(OpenClawBaseSkill):
    """
    Runway browser automation skill - Tier 1 easy
    Professional quality, Gen-4 models available
    """
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using Runway
        ...
        try:
            ...
            logger.info(f"[Runway] Generating video: {prompt[:50]}...")
            ...
            logger.info("[Runway] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[Runway] Video generated: {video_uri[:80] if video_uri else 'N/A'}..."
            )
            ...
            logger.error(f"[Runway] Generation failed: {str(e)}")
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "runway"}
