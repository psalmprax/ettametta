...
class MorphStudioSkill(OpenClawBaseSkill):
    """
    Morph Studio browser automation skill - Tier 2 moderate
    AI video generation platform with creative controls
    """

    def __init__(self):
        super().__init__()
        self.base_url = "https://morphstudio.com"
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using Morph Studio
        ...
        try:
            ...
            logger.info(f"[Morph Studio] Generating video: {prompt[:50]}...")
            ...
            logger.info("[Morph Studio] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[Morph Studio] Video generated: {video_uri[:80] if video_uri else 'N/A'}..."
            )
            ...
            logger.error(f"[Morph Studio] Generation failed: {str(e)}")
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "morph"}
