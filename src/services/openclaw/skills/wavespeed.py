...
class WaveSpeedAISkill:
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using WaveSpeedAI
        ...
        try:
            ...
            logger.info(f"[WaveSpeedAI] Generating video: {prompt[:50]}...")
            ...
            logger.info("[WaveSpeedAI] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[WaveSpeedAI] Video generated: {video_url[:80] if video_url else 'N/A'}..."
            )
            ...
            logger.error(f"[WaveSpeedAI] Generation failed: {str(e)}")
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "wavespeed"}
...
wavespeed_skill = WaveSpeedAISkill()
