...
class PikaSkill(OpenClawBaseSkill):
    """
    Pika browser automation skill - Tier 1 easy
    150 free credits monthly with unique "pikaffects" (crush, melt, inflate)
    Great for creative viral effects
    """

    def __init__(self):
        super().__init__()
        self.base_url = "https://pika.art"
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        ...
        try:
            ...
            logger.info("[Pika] Generation submitted, waiting for render...")
            ...
            logger.error(f"[Pika] Generation failed: {str(e)}")
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "pika"}
