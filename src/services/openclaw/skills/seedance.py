...
class SeedanceSkill(OpenClawBaseSkill):
    """
    Seedance browser automation skill - Tier 1 easy
    Built-in audio, dialogue, music support
    """

    def __init__(self):
        super().__init__()
        self.base_url = "https://seedance.ai"
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using Seedance
        ...
        try:
            ...
            logger.info(f"[Seedance] Generating video: {prompt[:50]}...")
            ...
            logger.info("[Seedance] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[Seedance] Video generated: {video_uri[:80] if video_uri else 'N/A'}..."
            )
            ...
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "seedance"}
