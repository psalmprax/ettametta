...
class ViduSkill(OpenClawBaseSkill):
    """
    Vidu browser automation skill - Tier 1 easy
    Multi-shot narrative support
    """

    def __init__(self):
        super().__init__()
        self.base_url = "https://vidu.ai"
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using Vidu
        ...
        try:
            ...
            logger.info(f"[Vidu] Generating video: {prompt[:50]}...")
            ...
            logger.info("[Vidu] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[Vidu] Video generated: {video_uri[:80] if video_uri else 'N/A'}..."
            )
            ...
            logger.error(f"[Vidu] Generation failed: {str(e)}")
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "vidu"}
