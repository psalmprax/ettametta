...
class HeyGenSkill(OpenClawBaseSkill):
    """
    HeyGen browser automation skill - Tier 2 moderate
    AI avatars, business videos (3 free 720p videos/month)
    """

    def __init__(self):
        super().__init__()
        self.base_url = "https://heygen.com"
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using HeyGen
        ...
        try:
            ...
            logger.info(f"[HeyGen] Generating video: {prompt[:50]}...")

            await self.page.goto(f"{self.base_url}/ai-video generator")
            ...
            logger.info("[HeyGen] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[HeyGen] Video generated: {video_uri[:80] if video_uri else 'N/A'}..."
            )
            ...
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "heygen"}
