...
class HailuoSkill(OpenClawBaseSkill):
    """
    Hailuo AI browser automation skill - Tier 1 easy
    Good human motion and action scenes
    """

    def __init__(self):
        super().__init__()
        self.base_url = "https://hailuoml.com"
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using Hailuo
        ...
        try:
            ...
            logger.info(f"[Hailuo] Generating video: {prompt[:50]}...")
            ...
            logger.info("[Hailuo] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[Hailuo] Video generated: {video_url[:80] if video_url else 'N/A'}..."
            )
            ...
            logger.error(f"[Hailuo] Generation failed: {str(e)}")
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "hailuo"}
