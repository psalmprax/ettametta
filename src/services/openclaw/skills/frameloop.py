...
class FrameloopSkill(OpenClawBaseSkill):
    """
    Frameloop browser automation skill - Tier 1 easy
    No watermark on free tier, good for faceless content
    """

    def __init__(self):
        super().__init__()
        self.base_url = "https://frameloop.ai"
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using Frameloop
        ...
        try:
            ...
            logger.info(f"[Frameloop] Generating video: {prompt[:50]}...")
            ...
            logger.info("[Frameloop] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[Frameloop] Video generated: {video_url[:80] if video_url else 'N/A'}..."
            )
            ...
            logger.error(f"[Frameloop] Generation failed: {str(e)}")
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "frameloop"}
...
frameloop_skill = FrameloopSkill()
