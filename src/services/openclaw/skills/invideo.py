...
class InVideoAISkill(OpenClawBaseSkill):
    """
    InVideo AI browser automation skill - Tier 1 easy platform
    AI-powered video creation from text prompts
    """

    def __init__(self):
        super().__init__()
        self.base_url = "https://invideo.io"
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using InVideo AI
        ...
        try:
            ...
            logger.info(f"[InVideo AI] Generating video: {prompt[:50]}...")

            await self.page.goto(f"{self.base_url}/video-generator")
            ...
            generate_btn = self.page.get_by_role(
                "button", name=lambda x: "Generate" in x or "Create" in x or "AI" in x
            )
            await generate_btn.click()

            logger.info("[InVideo AI] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[InVideo AI] Video generated: {video_url[:80] if video_url else 'N/A'}..."
            )
            ...
            logger.error(f"[InVideo AI] Generation failed: {str(e)}")
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "invideo"}
