...
class LeonardoSkill(OpenClawBaseSkill):
    """
    Leonardo AI browser automation skill - Tier 1 easy
    Also has image-to-video, combined workflow
    """

    def __init__(self):
        super().__init__()
        self.base_url = "https://leonardo.ai"
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using Leonardo
        ...
        try:
            ...
            logger.info(f"[Leonardo] Generating video: {prompt[:50]}...")

            await self.page.goto(f"{self.base_url}/platform/ai-video")
            ...
            logger.info("[Leonardo] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[Leonardo] Video generated: {video_url[:80] if video_url else 'N/A'}..."
            )
            ...
            logger.error(f"[Leonardo] Generation failed: {str(e)}")
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "leonardo"}
