...
class FlikiSkill(OpenClawBaseSkill):
    """
    Fliki browser automation skill - Tier 1 easy platform
    Text-to-video with voiceover support
    """

    def __init__(self):
        super().__init__()
        self.base_url = "https://fliki.ai"
    ...
    async def generate(self, prompt: str, aspect_ratio: str = "16:9") -> dict[str, Any]:
        """
        Generate video from prompt using Fliki
        ...
        try:
            ...
            logger.info(f"[Fliki] Generating video: {prompt[:50]}...")

            await self.page.goto(f"{self.base_url}/new")
            ...
            logger.info("[Fliki] Generation submitted, waiting for render...")
            ...
            logger.info(
                f"[Fliki] Video generated: {video_url[:80] if video_url else 'N/A'}..."
            )
            ...
            logger.error(f"[Fliki] Generation failed: {str(e)}")
            await self.cleanup()
            return {"status": "failed", "error": str(e), "engine": "fliki"}
