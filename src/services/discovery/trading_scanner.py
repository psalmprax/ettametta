...
class TradingScanner:
    ...
    async def scan_trends(self, niche: str, published_after: Optional[datetime] = None) -> List[ContentCandidate]:
        ...
        financial_niches = ["crypto", "finance", "stocks", "trading", "economy", "money", "bitcoin"]
        ...
        try:
            ...
            for symbol in tickers:
                ...
                change = 0
                if "stock_quote" in sentiment_data and sentiment_data["stock_quote"].get("change_percent"):
                    change = abs(float(sentiment_data["stock_quote"]["change_percent"].replace("%", "")))
                ...
                if change >= 5:
                    ...
                    candidates.append(ContentCandidate(
                        ...
                        url=f"https://www.coingecko.com/en/coins/{symbol}" if symbol.islower() else f"https://finance.yahoo.com/quote/{symbol}",
                        author="Market Pulse",
            ...
            if candidates:
                logger.info(f"[TradingScanner] Identified {len(candidates)} market signals for niche: {niche}")
