...
class AIJobOrchestrator:
    def __init__(self):
        ...
        print(
            "🚀 AI Job Orchestrator initialized (Smart Batching enabled).", flush=True
        )
    ...
    def _worker_loop(self):
        while self.is_running:
            ...
            with self.lock:
                ...
                current_model = model_manager.current_model_key or (
                    list(VIDEO_MODELS.keys())[0] if VIDEO_MODELS else None
                )
                    ...
                    for i, job in enumerate(self.pending_jobs):
                        if job.model_key == current_model:
                            ...
                            if self.pending_jobs[0].jumps < self.max_jumps:
                                best_idx = i
                                print(
                                    f"✨ [Batching] Promoting job {job.job_id} ({job.model_key}) to skip VRAM swap",
                                    flush=True,
                                )
                                break
                            else:
                                print(
                                    f"⚠️ [Batching] Starvation limit reached for {self.pending_jobs[0].job_id}. Forcing FIFO jump.",
                                    flush=True,
                                )
                                best_idx = 0
                                break
    ...
    def _execute_job(self, job: AIJob):
        ...
        try:
            if job.job_type == "video":
                job.result = ai_actions.action_render_video(
                    job.job_id, job.model_key, job.data
                )
            elif job.job_type == "voice":
                job.result = ai_actions.action_generate_voice(job.data.text)
            elif job.job_type == "vlm":
                job.result = ai_actions.action_analyze_vlm(
                    job.data.image_base64, job.data.prompt
                )
