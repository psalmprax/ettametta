...
async def get_trading_summary(
    ...
    try:
        # Get portfolio
        stmt = select(TradingPortfolioDB).filter(TradingPortfolioDB.user_id == current_user.id)
...
async def add_to_watchlist(
    ...
    try:
        # Check existing
        stmt = select(TradingWatchlistDB).filter(
            and_(TradingWatchlistDB.user_id == current_user.id, TradingWatchlistDB.symbol == request.symbol)
        )
        result = await db.execute(stmt)
...
async def remove_from_watchlist(
    ...
    try:
        symbol = validate_symbol(symbol)
        stmt = delete(TradingWatchlistDB).filter(
            and_(TradingWatchlistDB.user_id == current_user.id, TradingWatchlistDB.symbol == symbol)
        )
