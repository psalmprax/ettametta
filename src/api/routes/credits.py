...
async def _get_packages(db: AsyncSession) -> list[dict]:
...
async def get_credit_balance(
    ...
    return success_response(data={"balance": balance, "user_id": current_user.id})
...
async def purchase_credits(
    ...
    if not package:
        available_ids = [p["id"] for p in await _get_packages(db)]
        raise HTTPException(
            status_code=404,
...
async def get_credit_costs(current_user: UserDB = Depends(get_current_user)):
    ...
    return success_response(data={"tier": tier, "costs": costs})
...
async def get_referral_code(
    ...
    return success_response(data={"referral_code": code, "share_url": share_url})
...
async def apply_referral_code(
    ...
    success, message = await credit_service.apply_referral_code(
        current_user.id, request.referral_code.strip().upper(), db
    )

    if not success:
        raise HTTPException(status_code=400, detail=message)

    return success_response(data={"status": "success", "message": message})
...
async def get_referrals(
    ...
    return success_response(data={"referrals": referrals, "stats": stats})
