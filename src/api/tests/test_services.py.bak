...
class TestCrewAIService:
    ...
    @patch.dict(
        os.environ, {"ENABLE_CREWAI": "false", "CREWAI_AGENTS": "researcher,writer"}
    )
...
class TestAffiliateService:
    ...
    @patch.dict(
        os.environ,
        {"ENABLE_AFFILIATE_API": "true", "AMAZON_ASSOCIATES_TAG": "test_tag"},
    )
    ...
    def test_generate_affiliate_link_amazon(self):
        ...
        url = service.generate_affiliate_link("https://amazon.com/product", "amazon")
...
class TestTradingService:
    ...
    @patch.dict(
        os.environ, {"ENABLE_TRADING": "true", "ALPHA_VANTAGE_API_KEY": "test_key"}
    )
    def test_service_enabled_with_api_keys(self):
...
class TestInterpreterService:
    ...
    @patch.dict(
        os.environ, {"ENABLE_INTERPRETER": "true", "INTERPRETER_SANDBOX": "true"}
    )
    ...
    @patch.dict(
        os.environ, {"ENABLE_INTERPRETER": "true", "INTERPRETER_MAX_RUNTIME": "30"}
    )
