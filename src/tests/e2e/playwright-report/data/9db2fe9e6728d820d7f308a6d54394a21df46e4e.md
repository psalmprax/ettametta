...
- Name: nexus/nexus_composition.spec.ts >> Nexus Composition - Blueprint Templates >> should filter templates by category
- Location: tests/nexus/nexus_composition.spec.ts:144:9
...
> 133 |         await page.fill('input[name="email"]', 'test@example.com');
  ...
  159 |         await page.click('[data-testid="template-card"]:first-child');
  160 |         await page.fill('input[name="title"]', 'My Custom Title');
  161 |         await page.click('button:has-text("Save")');
  162 |         await expect(page.locator('[data-testid="template-saved"]')).toBeVisible();
  163 |     });
  164 | });
```