...
- Name: publishing_flow.spec.ts >> Multi-Platform Publishing >> should post to multiple platforms at once
- Location: tests/publishing_flow.spec.ts:99:7
...
> 92  |     await page.goto('/login');
  ...
  107 |     await page.check('input[name="platform-tiktok"]');
  108 |     await page.check('input[name="platform-instagram"]');
  109 |     
  110 |     await page.click('button:has-text("Publish All")');
  111 |     
  112 |     await expect(page.locator('[data-testid="multi-post-success"]')).toBeVisible();
  113 |   });
  114 | });
```