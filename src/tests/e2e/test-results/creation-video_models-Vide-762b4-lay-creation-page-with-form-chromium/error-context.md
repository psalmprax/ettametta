...
- Name: creation/video_models.spec.ts >> Video Model - Dashboard UI Tests >> should display creation page with form
- Location: tests/creation/video_models.spec.ts:147:9
...
```ts
  48  |         try {
  49  |             const response = await request.post(`${GPU_SERVER_URL}/generate`, {
  50  |                 data: {
  51  |                     prompt: 'A test video',
  52  |                     engine: 'cogvideo',
  53  |                     style: 'cinematic',
  54  |                     aspect_ratio: '16:9'
  55  |                 },
  56  |                 timeout: 10000
  57  |             });
  58  |             console.log('CogVideo response:', response.status());
  59  |         } catch (e) {
  60  |             console.log('CogVideo endpoint test - server may be busy');
  61  |         }
  62  |     });
  63  | 
  64  |     test('should test HunyuanVideo model endpoint', async ({ request }) => {
  65  |         try {
  66  |             const response = await request.post(`${GPU_SERVER_URL}/generate`, {
  67  |                 data: {
  68  |                     prompt: 'A test video',
  69  |                     engine: 'hunyuan',
  70  |                     style: 'cinematic',
